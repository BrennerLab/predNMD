...
# Ensembl VEP configuration (OPTIONAL, only required if the input VCF is not annotated by Ensembl VEP)
vep:
...
- `MECH_CLASS`:  Mechanism classification for variants predicted as not triggering NMD 
...
- Can be VCF either unannotated (will run Ensembl VEP automatically, **note that the VCF should represent the variant in Ensembl-reference style, i.e., without "chr" prefix in CHR column**) or annotated with Ensembl VEP (must include allele frequency annotation)
